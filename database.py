...
# MongoDB Connection
# MongoDB Connection
MONGO_URL = os.getenv("MONGO_URL", "mongodb+srv://ankitkrit32_db_user:hfFUtCd708yze49r@cluster0.nu4wmrr.mongodb.net/?appName=Cluster0")
DB_NAME = os.getenv("DB_NAME", "ankitkrit32_db_user")
...
async def init_db():
    ...
    try:
        ...
        print("✅ 𝐋𝐔𝐂𝐈 𝐂𝐇𝐊 𝐃𝐀𝐓𝐀𝐁𝐀𝐒𝐄 𝐈𝐍𝐈𝐓𝐈𝐀𝐋𝐈𝐙𝐄𝐃 𝐒𝐔𝐂𝐂𝐄𝐒𝐒𝐅𝐔𝐋𝐋𝐘!")
...
async def get_user_limit(user_id: int, plan: str) -> int:
    """𝗚𝗲𝘁 𝘂𝘀𝗲𝗿'𝘀 𝗼𝗻𝗲-𝘁𝗶𝗺𝗲 𝗰𝗵𝗲𝗰𝗸 𝗹𝗶𝗺𝗶𝘁"""
    user = await users_col.find_one({"user_id": user_id})
...
async def set_user_limit(user_id: int, limit: int):
    """𝗦𝗲𝘁 𝗰𝘂𝘀𝘁𝗼𝗺 𝗼𝗻𝗲-𝘁𝗶𝗺𝗲 𝗰𝗵𝗲𝗰𝗸 𝗹𝗶𝗺𝗶𝘁 for user"""
    await users_col.update_one(
        {"user_id": user_id},
        {"$set": {
            "custom_limit": limit,
...
async def create_redeem_key(key: str, plan: str, days: int, limit: int):
    """𝗖𝗿𝗲𝗮𝘁𝗲 𝗮 𝗿𝗲𝗱𝗲𝗲𝗺 𝗸𝗲𝘆"""
...
async def redeem_key(key: str, user_id: int):
    """𝗥𝗲𝗱𝗲𝗲𝗺 𝗮 𝗸𝗲𝘆 𝗳𝗼𝗿 𝗮 𝘂𝘀𝗲𝗿"""
    ...
    return True, {"plan": plan, "days": days, "limit": limit, "expiry": expiry}
